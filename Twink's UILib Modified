...
function UILibrary.Load(GUITitle)
    ...
    function TabLibrary.AddPage(PageTitle, SearchBarIncluded)
        ...
        function PageLibrary.AddButton(Text, Callback, Parent, Underline)
            local Button = {}
            local CallBackFunction = Callback
            ...
            function Button.Rehook(Text, CallbackNew)
        ...
        function PageLibrary.AddDropdown(Text, ConfigurationArray, Callback, Default)
            local DropdownData = {}
            ...
            local function AddOptions(OptionIndex, Option)
                DropdownButtons[OptionIndex] = PageLibrary.AddButton(Option, function()
                    Callback(Option)
                    DropdownLabel.Text = Text..": "..Option
                end, DropdownFrame, true)
            end
            for OptionIndex, Option in next, DropdownArray do
                AddOptions(OptionIndex, Option)
            end
            ...
            function DropdownData.SetArray(Array)
                DropdownArray = Array or {}
                if #DropdownArray == #DropdownButtons then
                    for index, Button in pairs(DropdownButtons) do
                        Button.Rehook(DropdownArray[index], function()
                        Callback(DropdownArray[index])
                        DropdownLabel.Text = Text..": "..DropdownArray[index]
                        end)
                    end
                    Tween(DropdownContainer, {Size = DropdownToggle and UDim2.new(1,0,0,20+(#DropdownArray*20)) or UDim2.new(1,0,0,20)})

                elseif #DropdownArray > #DropdownButtons then
                    for index, Option in pairs(DropdownArray) do
                        if index <= #DropdownButtons then
                            DropdownButtons[index].Rehook(Option, function()
                            Callback(Option)
                            DropdownLabel.Text = Text..": "..Option
                            end)
                        else
                ...
                elseif #DropdownArray < #DropdownButtons then
                    for index, Button in pairs(DropdownButtons) do
                        if index <= #DropdownArray then
                            ...
                            Button.Destroy()
                            DropdownArray[index] = nil
                            table.remove(DropdownButtons, index)
                        end
                    end
                    Tween(DropdownContainer, {Size = DropdownToggle and UDim2.new(1,0,0,20+(#DropdownButtons*20)) or UDim2.new(1,0,0,20)})
                end
            end
